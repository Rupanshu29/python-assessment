...
Answer:
Both .py and .ipynb files are used for writing Python code, but they are used in different ways.
.py File	.ipynb File
Python script file	Jupyter Notebook file
Contains only Python code	Contains code, output, graphs and text
Runs as a complete program	Runs cell by cell
Used for software development	Used for data analysis and machine learning
Points:
...
print("Hello World")
Along with the code, it can also show outputs, charts and explanations in separate cells.
...
Numerical Feature
A numerical feature contains measurable values and mathematical operations can be performed on it.
Examples:
•	Age
•	Cholesterol
•	Blood Pressure
...
Contains numbers	Contains categories
Used in calculations	Used for grouping
Example: Age, Cholesterol	Example: Sex, Chest Pain Type
Example:
Age	Cholesterol
45	220
60	280
...
________________________________________
3. You run type() on the target column and it returns int. Why does the data type of your target variable matter before framing a prediction problem?
Answer:
The data type of the target variable helps us understand what type of prediction problem we are solving.
...
Answer:
Correlation is a measure that shows the relationship between two variables. It tells us whether two variables move together and how strong that relationship is.
...
55	Yes
65	Yes
In this example, older individuals appear more likely to have heart disease, but there can be exceptions.
________________________________________
...
Answer:
A rule-based system and a data-driven approach are two different ways of making decisions.
Rule-Based System
...
Example:
A patient may have cholesterol below 240 but still be at high risk because of age and blood pressure. A data-driven model can identify such cases using historical data.
Rule-Based System	Data-Driven Approach
Uses fixed rules	Learns from data
Created manually	Learns automatically
Does not improve by itself	Improves with more data
